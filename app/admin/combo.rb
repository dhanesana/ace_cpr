...
# end
  ...
  form do |f|
    f.inputs "Combo Details" do
      f.input :primary_id, as: :select, collection: Appointment.all.collect { |appt| next if appt == nil; next if appt.type == nil; ["#{appt.formated_date} => #{appt.type.name}", appt.id] }
      f.input :secondary_id, as: :select, collection: Appointment.all.collect { |appt| next if appt == nil; next if appt.type == nil; ["#{appt.formated_date} => #{appt.type.name}", appt.id] }
    end
    f.actions
  end
